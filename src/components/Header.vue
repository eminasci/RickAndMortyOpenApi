<template>
  <header class="header">
    <div class="header-container">
      <a href="#" @click="reloadPage"><img src="https://upload.wikimedia.org/wikipedia/commons/b/b1/Rick_and_Morty.svg" alt="Rick and Morty Logo" class="logo"></a>
...
.header {
  ...
  padding: 20px 0;
  position: relative; /* Sabit pozisyon yok */
  ...
  transition: top 0.3s; /* Header kaydırma efektini ayarlar */
}
...
.logo {
  height: 60px;
  filter: drop-shadow(0 0 10px #00ff00); /* Neon yeşil parıltı */
}
...
.nav ul li {
  margin-left: 20px; /* Her bir menü öğesi arasındaki boşluk */
}
...
.nav ul li a {
  ...
  padding: 10px 15px; /* Yatay ve dikey boşluk */
...
.nav ul li a:hover {
  background-color: #00ff00; /* Hover durumunda arka plan rengi */
  color: #1b1b1b; /* Hover durumunda metin rengi */
  transform: scale(1.1); /* Hover durumunda büyütme efekti */
}

.nav ul li a:active {
  transform: scale(0.9); /* Tıklama durumunda küçültme efekti */
}
...
@media (max-width: 768px) {
  ...
  .nav.nav-open {
    max-height: 300px; /* Menü yüksekliğini ayarla */
  }
